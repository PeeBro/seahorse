...
write_csv(norm.be.replicate$estimates, paste0(OUTPUT_PATH,"/OCR-Estimated_values.csv"))
write_csv(log.be.replicate$estimates, paste0(OUTPUT_PATH,"/LOCR-Estimates_values.csv"))
write_csv(norm.be.replicate$bioenergetics, paste0(OUTPUT_PATH,"/OCR-BE-table.csv"))
write_csv(log.be.replicate$bioenergetics, paste0(OUTPUT_PATH,"/LOCR-BE-table.csv"))
...
n.bio.well <- melt(norm.be.well$bioenergetics)
n.bio.well <- n.bio.well %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                     Group = substr(Sample, start = 1, stop = nchar(Sample)-19))

n.bio.replicate <- melt(norm.be.replicate$bioenergetics)
n.bio.replicate <- n.bio.replicate %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                               Group = substr(Sample, start = 1, stop = nchar(Sample)-19))


n.bio.sample <- melt(norm.be.sample$bioenergetics)
n.bio.sample.se <- melt(norm.be.sample$standard.errors)
colnames(n.bio.sample.se)[3] <- "se"
n.bio.sample <- merge(n.bio.sample,n.bio.sample.se)
n.bio.sample <- n.bio.sample %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)))
```
...
n.bio.well %>%
  ...
  geom_point(data = n.bio.replicate, mapping = aes(x = "", y = value, group = Group, fill = Date), 
             color = "black", shape = 21, size = 5, 
             position = position_dodge(width = 0.8))+
  geom_point(data = n.bio.sample, mapping = aes(x = "", y = value, group = Sample), 
             fill = "black", color = "red", shape = 21, size = 1.5,
             position = position_dodge(width = 0.8))+
  geom_errorbar(data = n.bio.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
...
n.bio.replicate %>%
  ...
  geom_point(data = n.bio.sample, mapping = aes(x = variable, y = value, group = Sample), color = "black", size = 1.5,
...
n.bio.well %>%
  ...
  ggtitle("Difference based natural scale Bio-Energetics OCR")+
  geom_boxplot(aes(fill = Group, color = Group), alpha = 0.5, position = position_dodge(width = 0.6),)+
  geom_point(aes(color = Group), position = position_dodge(width = 0.6), size = 1)+
  # geom_point(data = n.bio.sample, mapping = aes(x = variable, y = value, group = Sample), color = "black", size = 3,
  #            position = position_dodge(width = 0.6))+
  # geom_errorbar(data = n.bio.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
  #                position=position_dodge(0.6))+
  scale_color_brewer(palette = "Dark2")+
...
l.bio.well <- melt(log.be.well$bioenergetics)
l.bio.well <- l.bio.well %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                     Group = substr(Sample, start = 1, stop = nchar(Sample)-19))

l.bio.replicate <- melt(log.be.replicate$bioenergetics)
l.bio.replicate <- l.bio.replicate %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                               Group = substr(Sample, start = 1, stop = nchar(Sample)-19))


l.bio.sample <- melt(log.be.sample$bioenergetics)
l.bio.sample.se <- melt(log.be.sample$standard.errors)
colnames(l.bio.sample.se)[3] <- "se"
l.bio.sample <- merge(l.bio.sample,l.bio.sample.se)
l.bio.sample <- l.bio.sample %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)))
```
...
l.bio.well %>%
  ...
  geom_beeswarm(aes(color = Date, shape = Group, group = Group), cex = 3, dodge.width = 0.8)+
  geom_point(data = l.bio.replicate, mapping = aes(x = "", y = value, group = Group, fill = Date), 
             color = "black", shape = 21, size = 5, 
             position = position_dodge(width = 0.8))+
  geom_point(data = l.bio.sample, mapping = aes(x = "", y = value, group = Sample), 
             fill = "black", color = "red", shape = 21, size = 1.5,
             position = position_dodge(width = 0.8))+
  geom_errorbar(data = l.bio.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.8))+
  scale_shape_manual(values= c(15,8, 17,3))+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
...
l.bio.replicate %>%
  ...
  # geom_boxplot(width = 0.5, outlier.size = -1, alpha = 0.7)+
  geom_point(aes(fill = Group, shape = Date), size = 3, position = position_dodge(width = 0.6))+
  geom_point(data = l.bio.sample, mapping = aes(x = variable, y = value, group = Sample), color = "black", size = 1.5,
             position = position_dodge(width = 0.6))+
  geom_errorbar(data = l.bio.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.6))+
  scale_shape_manual(values= c(21,22))+
...
write_csv(n.bio.replicate, paste0(OUTPUT_PATH,"/OCR-BioEnergetics.csv"))
...
# be.er <- arrange(be.er, Sample)
write_csv(l.bio.replicate, paste0(OUTPUT_PATH,"/LogOCR-BioEnergetics.csv"))
...
write_csv(norm.be.PER.replicate$estimates, paste0(OUTPUT_PATH,"/PER-Estimated_values.csv"))
write_csv(log.be.PER.replicate$estimates, paste0(OUTPUT_PATH,"/LPER-Estimated_values.csv"))
write_csv(norm.be.PER.replicate$bioenergetics, paste0(OUTPUT_PATH,"/PER-BE-table.csv"))
write_csv(log.be.PER.replicate$bioenergetics, paste0(OUTPUT_PATH,"/LPER-BE-table.csv"))
```
### Estimates
...
n.bio.PER.well <- melt(norm.be.PER.well$bioenergetics)
n.bio.PER.well <- n.bio.PER.well %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                     Group = substr(Sample, start = 1, stop = nchar(Sample)-19))

n.bio.PER.replicate <- melt(norm.be.PER.replicate$bioenergetics)
n.bio.PER.replicate <- n.bio.PER.replicate %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                               Group = substr(Sample, start = 1, stop = nchar(Sample)-19))


n.bio.PER.sample <- melt(norm.be.PER.sample$bioenergetics)
n.bio.PER.sample.se <- melt(norm.be.PER.sample$standard.errors)
colnames(n.bio.PER.sample.se)[3] <- "se"
n.bio.PER.sample <- merge(n.bio.PER.sample,n.bio.PER.sample.se)
n.bio.PER.sample <- n.bio.PER.sample %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)))
```
...
n.bio.PER.well %>%
  ...
  geom_beeswarm(aes(color = Date, shape = Group, group = Group), cex = 3, dodge.width = 0.8)+
  geom_point(data = n.bio.PER.replicate, mapping = aes(x = "", y = value, group = Group, fill = Date), 
             color = "black", shape = 21, size = 5, 
             position = position_dodge(width = 0.8))+
  geom_point(data = n.bio.PER.sample, mapping = aes(x = "", y = value, group = Sample), 
             fill = "black", color = "red", shape = 21, size = 1.5,
             position = position_dodge(width = 0.8))+
  geom_errorbar(data = n.bio.PER.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.8))+
  scale_shape_manual(values= c(15,8, 17,3))+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
...
n.bio.PER.replicate %>%
  ...
  # geom_boxplot(width = 0.5, outlier.size = -1, alpha = 0.7)+
  geom_point(aes(fill = Group, shape = Date), size = 3, position = position_dodge(width = 0.6))+
  geom_point(data = n.bio.PER.sample, mapping = aes(x = variable, y = value, group = Sample), color = "black", size = 1.5,
             position = position_dodge(width = 0.6))+
  geom_errorbar(data = n.bio.PER.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.6))+
  scale_shape_manual(values= c(21,22))+
...
l.bio.PER.well <- melt(log.be.PER.well$bioenergetics)
l.bio.PER.well <- l.bio.PER.well %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                     Group = substr(Sample, start = 1, stop = nchar(Sample)-19))

l.bio.PER.replicate <- melt(log.be.PER.replicate$bioenergetics)
l.bio.PER.replicate <- l.bio.PER.replicate %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)),
                                               Group = substr(Sample, start = 1, stop = nchar(Sample)-19))


l.bio.PER.sample <- melt(log.be.PER.sample$bioenergetics)
l.bio.PER.sample.se <- melt(log.be.PER.sample$standard.errors)
colnames(l.bio.PER.sample.se)[3] <- "se"
l.bio.PER.sample <- merge(l.bio.PER.sample,l.bio.PER.sample.se)
l.bio.PER.sample <- l.bio.PER.sample %>%  mutate(Date = substr(Sample, start = nchar(Sample)-15, stop = nchar(Sample)))
```
...
l.bio.PER.well %>%
  filter(variable != "Other") %>%
  ggplot(aes(x = "", y = value))+
  ggtitle("Ratio based log scale Bio-Energetics PER (folds)")+
  geom_beeswarm(aes(color = Date, shape = Group, group = Group), cex = 3, dodge.width = 0.8)+
  geom_point(data = l.bio.PER.replicate, mapping = aes(x = "", y = value, group = Group, fill = Date), 
             color = "black", shape = 21, size = 5, 
             position = position_dodge(width = 0.8))+
  geom_point(data = l.bio.PER.sample, mapping = aes(x = "", y = value, group = Sample), 
             fill = "black", color = "red", shape = 21, size = 1.5,
             position = position_dodge(width = 0.8))+
  geom_errorbar(data = l.bio.PER.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.8))+
  scale_shape_manual(values= c(15,8, 17,3))+
  scale_color_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2")+
...
l.bio.PER.replicate %>%
  ...
  # geom_boxplot(width = 0.5, outlier.size = -1, alpha = 0.7)+
  geom_point(aes(fill = Group, shape = Date), size = 3, position = position_dodge(width = 0.6))+
  geom_point(data = l.bio.PER.sample, mapping = aes(x = variable, y = value, group = Sample), color = "black", size = 1.5,
             position = position_dodge(width = 0.6))+
  geom_errorbar(data = l.bio.PER.sample, aes(ymin=value-se, ymax=value+se, group = Sample), width=0.5, size = 0.8,
                 position=position_dodge(0.6))+
  scale_shape_manual(values= c(21,22))+
...
# be.er  <- arrange(be.er, Sample)
write_csv(n.bio.PER.replicate, paste0(OUTPUT_PATH,"/PER-BioEnergetics.csv"))
# add standard errors
# st.err <- melt(log.be$standard.errors, value.name = "SE", variable.name = "variable" )
# be.er  <- left_join(l.bio.PER, st.err, by = c("Sample", "variable"))
# be.er <- arrange(be.er, Sample)
write_csv(l.bio.PER.replicate, paste0(OUTPUT_PATH,"/logPER-BioEnergetics.csv"))
