...
params:
  input_folder: "3 intervals2_test"
  error_measure: "se"
editor_options: 
...
basal.PER.sample <- n.bio.PER.sample %>%
  filter(variable %in% c("Basal.PER"))
total.min.sample <- merge(x = basal.OCR.sample, y = basal.PER.sample,by = c("Sample"), suffixes = c(".OCR", ".PER"))
...
max.OCR.sample <- n.bio.sample %>%
  filter(variable %in% c("Maximal.Resp"))
total.max.sample <- merge(x = max.PER.sample, y = max.OCR.sample,by = c("Sample"), suffixes = c(".PER", ".OCR"))
...
total.well %>% ggplot()+
  ...
  geom_point(mapping = aes(y = mean.PER, x = mean.OCR, fill = Group.PER), shape = 21)+
  facet_wrap(~min_max)+
  geom_point(data = total.sample, aes(y = mean.PER, x = mean.OCR, fill = Sample), size = 4, shape = 21)+
