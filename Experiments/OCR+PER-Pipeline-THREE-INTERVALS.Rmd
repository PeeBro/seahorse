...
Steps performed: <br>
- Check that the number of intervals for each sample is three and that no sample has an interval with less than 18 measurements <br>
...
# number of intervals has to be 3 or the well discarted 
inter <- d_OCR %>% 
  group_by(sample_id) %>% 
...
cat("Replicates removed because they have less or more than three intervals: ", paste(unique(inter$sample_id), collapse = "\n"), sep = "\n")
```
...
summary_plot_function2(log.be.well, log.be.replicate, log.be.sample)+
...
# filter out samples with any of intervals having less than 18 measurements
sumar <- d_PER %>%
  group_by(sample_id, Interval) %>%
  summarise(n = n()) %>%
  filter(n < 18)
...
for (smpl in unique(dr_per$sample_id)) {
  d    <- filter(dr_per, sample_id == smpl)
  ptg  <- nrow(filter(d, is.out.p == T))/nrow(d)*100
  ...
  d    <- filter(dr_ocr, sample_id == smpl)
  ptg  <- nrow(filter(d, is.out.p == T))/nrow(d)*100
  plot <- ggplot(d, aes(Measurement, OCR))+
            ggtitle(paste0("ID: ", smpl, " |  Outliers: ", format(ptg, digits = 3), "%"))+
            geom_line(aes(group = Well),size = 0.2, color = "grey") +
            geom_point(aes(Measurement, OCR, color = is.out.p))+
            guides(color=guide_legend(title="Outlier"))+
            xlab("Measurement")+
