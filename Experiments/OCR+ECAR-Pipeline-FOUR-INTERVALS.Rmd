---
title: "OCR+ECAR-Pipeline-FOUR-INTERVALS"
output: 
  html_document:
...
Step 2: Write the name of the folder above. Use quotes like this "My_folder" <br>
Step 3: Run the code chunk below <br>
...
```{r echo=FALSE}
# perep for ECAR interval 1 not used  
d_ECAR <- dm %>% filter(!Interval %in% c("Int1"))
                        
# filter out samples with any of intervals having less than 8 measurements 
sumar <- d_ECAR %>%
  group_by(sample_id, Interval) %>% 
  summarise(n = n()) %>% 
  filter(n < 8 | Interval == "Int2" & n < 18 | Interval == "Int3" & n < 18 |Interval == "Int4" & n < 12 )
