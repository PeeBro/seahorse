...
compute_bioenergetics_replicate <- function(dm_r, method) {
  ...
  if (method == "OCR") {
    ...
    sd_errors <- sd_n %>% 
      mutate(Sample           = sd.sample_id,
             Basal.Resp       = sqrt(((sd.Int1^2)+(sd.Int4^2))/2),
             ATP.linked.Resp  = sqrt(((sd.Int1^2)+(sd.Int2^2))/2),
             Proton.Leak      = sqrt(((sd.Int2^2)+(sd.Int4^2))/2),
             Non.Mito.Resp    = sd.Int4,
             mitoATP.prod.rate = sqrt(((sd.Int1^2)+(sd.Int2^2))/2),
             glycoATP.prod.rate = sqrt(((sd.Int1^2)+(sd.Int4^2)+(sd.Int1.PER^2))/3)) %>%
      select(c("Sample", "Basal.Resp", "ATP.linked.Resp", "Proton.Leak", "Non.Mito.Resp", "mitoATP.prod.rate", "glycoATP.prod.rate"))
    ...
    sd_errors <- sd_n %>% 
      mutate(Sample           = sd.sample_id,
             log.Basal.Resp       = sqrt(((sd.Int1^2)+(sd.Int4^2))/2),
             log.ATP.linked.Resp  = sqrt(((sd.Int1^2)+(sd.Int2^2))/2),
             log.Proton.Leak      = sqrt(((sd.Int2^2)+(sd.Int4^2))/2),
             log.Non.Mito.Resp    = sd.Int4,
             log.mitoATP.prod.rate = sqrt(((sd.Int1^2)+(sd.Int2^2))/2),
             log.glycoATP.prod.rate = sqrt(((sd.Int1^2)+(sd.Int4^2)+(sd.Int1.PER^2))/3)) %>%
      select(c("Sample", "log.Basal.Resp", "log.ATP.linked.Resp", "log.Proton.Leak", "log.Non.Mito.Resp", "log.mitoATP.prod.rate", "log.glycoATP.prod.rate"))
    ...
    sd_errors <- sd_n %>%
      mutate(Sample            = sd.sample_id,
             ...
             Glyco.Rsrv.Cpcty  = sqrt((sd.Int2^2+sd.Int1^2)/2)) %>%
      select(c("Sample", "Basal.PER", "Max.PER", "Glyco.Rsrv.Cpcty"))
  } else if (method == "LPER") {
    bio_e <- estimates %>%
      mutate(Sample                = sample_id,
    ...
    sd_errors <- sd_n %>%
      mutate(Sample            = sd.sample_id,
             ...
             log.Glyco.Rsrv.Cpcty  = sqrt((sd.Int2^2+sd.Int1^2)/2)) %>%
      select(c("Sample", "log.Basal.PER", "log.Max.PER", "log.Glyco.Rsrv.Cpcty"))
  }
